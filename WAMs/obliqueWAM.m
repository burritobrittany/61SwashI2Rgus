...
L=string(ls(fullfile(odir,'Datasets',fname,'RAW')));
L=L(3:end,:);
...
Io=imread(fullfile(odir,'Datasets',fname,'RAW',L(1)));
[r c co]=size(Io);
...
I=double(rgb2gray(imread(fullfile(odir,'Datasets',fname,'RAW',L(k)))));
...
%% Save File (MAT)
oname=[num2str(t(1)*1000) '_win' num2str(awin) '_ovr' num2str(ovr)];
save(fullfile(odir,'Datasets',fname,'WAMS',[oname 'O.mat']),'Iwam','Ncounter','TbinS','awin','ovr','TbinS')
disp('saved Mat File')

%% Make A Movie
if aflag==1
    
v = VideoWriter(fullfile(odir,'Datasets',fname,'WAMS',[oname,'O.avi']));
v.FrameRate=10;
open(v)
